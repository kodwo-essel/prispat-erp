...
import FinancePrintView from "@/app/dashboard/finance/components/FinancePrintView";

export default function FinanceExportPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-slate-400 bg-white">
                ...
                <span className="text-xs font-bold uppercase tracking-widest">Preparing Document...</span>
            </div>
        );
    }
